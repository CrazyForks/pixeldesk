...
export default function CharactersPage() {
  ...
  const fetchCharacters = async (page = 1, searchTerm = search) => {
    ...
    try {
      ...
      const response = await fetch(`/api/pixel-dashboard/characters?${params}`)
      if (response.ok) {
  ...
  const handleBatchImport = async () => {
    ...
    try {
      const response = await fetch('/api/pixel-dashboard/characters/batch-import', {
        method: 'POST'
      })
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    try {
      const response = await fetch(`/api/pixel-dashboard/characters/${id}`, {
        method: 'DELETE'
      })
  ...
  const handleBatchDelete = async () => {
    ...
    try {
      const response = await fetch('/api/pixel-dashboard/characters/batch-delete', {
        method: 'POST',
        headers: {
  ...
  const handleOpenEdit = async (character: Character) => {
    ...
    // 获取角色详情和历史日志
    try {
      const response = await fetch(`/api/pixel-dashboard/characters/${character.id}`)
      if (response.ok) {
        const data = await response.json()
        if (data.success) {
  ...
  const handleSaveEdit = async () => {
    ...
    setSaving(true)
    try {
      const response = await fetch(`/api/pixel-dashboard/characters/${editingCharacter.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={() => router.push('/pixel-dashboard/characters/create')}
