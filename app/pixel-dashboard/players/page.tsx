...
export default function PlayersPage() {
  ...
  const fetchPlayers = async (page = 1, searchTerm = search) => {
    ...
    try {
      ...
      const response = await fetch(`/api/pixel-dashboard/players?${params}`)
      if (response.ok) {
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true)
    try {
      const response = await fetch(`/api/pixel-dashboard/players/${deletingId}`, {
        method: 'DELETE',
      })
      if (response.ok) {
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-4">
                      <button
                        onClick={() => router.push(`/pixel-dashboard/players/${player.id}`)}
